...
// GET /pasien - Get all patients (users with role=1)
pasienRoutes.get('/', authenticateUserToken, (req, res) => {
  const query = `
    SELECT
      u.id,
      u.nama as name,
...
// GET /pasien/search - Search patients
pasienRoutes.get('/search', authenticateUserToken, (req, res) => {
  const { q } = req.query;
  if (!q || q.trim() === '') {
    return res.json([]);
  }
...
// GET /pasien/:id - Get patient by ID
pasienRoutes.get('/:id', authenticateUserToken, (req, res) => {
  const { id } = req.params;
  const query = `
    SELECT
      u.id,
...
pasienRoutes.post('/', authenticateUserToken, (req, res) => {
...
pasienRoutes.put('/:id', authenticateUserToken, (req, res) => {
  const { id } = req.params;
  const patientData = req.body;
...
pasienRoutes.delete('/:id', authenticateUserToken, (req, res) => {
  const { id } = req.params;
